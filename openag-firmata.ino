...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  switch (command) {
    case CBAN:
      reply = "", id = "", value = "";
      if (argv[0]) { // Handle Get Methods
        // Extract id from data, data = id
        for (int index=1; index<argc; index++) {
            id += char(argv[index]);
        }

        // Pass id to handler and get reply
        reply = cban_handler.get(id);
      }
      else { // Handle Set Methods
        // Extract id & val from data, data = <id>:<val>
        int index, flag = 0;
        ...
        // Pass id & val to handler and get reply
        reply = cban_handler.set(id, value);
      }
